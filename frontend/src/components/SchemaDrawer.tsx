...
const DATABASE_SCHEMA: TableMetadata[] = [
  {
    table_name: 'users',
    description: 'User identity accounts and authenticated contact email addresses',
    columns: [
      { name: 'id', type: 'SERIAL', is_pk: true },
      { name: 'email', type: 'VARCHAR(255)', is_pii: true },
    ],
  },
  {
    table_name: 'customers',
    description: 'Enterprise customer profiles with loyalty membership tiers and privacy controls',
    columns: [
      { name: 'customer_id', type: 'SERIAL', is_pk: true },
      { name: 'user_id', type: 'INTEGER', foreign_table: 'users', foreign_column: 'id' },
  ...
  {
    table_name: 'products',
    description: 'Product catalog with items, pricing, and revenue identifiers',
    columns: [
      { name: 'product_id', type: 'SERIAL', is_pk: true },
      { name: 'name', type: 'VARCHAR(255)' },
  ...
  {
    table_name: 'orders',
    description: 'Customer order transactions, order totals, and creation timestamps',
    columns: [
      { name: 'order_id', type: 'SERIAL', is_pk: true },
      { name: 'customer_id', type: 'INTEGER', foreign_table: 'customers', foreign_column: 'customer_id' },
      { name: 'order_total', type: 'NUMERIC(10,2)' },
      { name: 'created_at', type: 'TIMESTAMPTZ' },
    ],
  },
  {
    table_name: 'order_items',
    description: 'Individual line items linking purchase orders to catalog products and quantities',
    columns: [
      { name: 'item_id', type: 'SERIAL', is_pk: true },
      { name: 'order_id', type: 'INTEGER', foreign_table: 'orders', foreign_column: 'order_id' },
      { name: 'product_id', type: 'INTEGER', foreign_table: 'products', foreign_column: 'product_id' },
      { name: 'quantity', type: 'INTEGER' },
    ],
  },
  {
    table_name: 'payments',
    description: 'Financial transaction records and processing timestamps for orders',
    columns: [
      { name: 'payment_id', type: 'SERIAL', is_pk: true },
      { name: 'order_id', type: 'INTEGER', foreign_table: 'orders', foreign_column: 'order_id' },
  ...
  },
  {
    ...
    description: 'Order fulfillment status (Delivered, Processing) and shipment timestamps',
    columns: [
      { name: 'shipment_id', type: 'SERIAL', is_pk: true },
      { name: 'order_id', type: 'INTEGER', foreign_table: 'orders', foreign_column: 'order_id' },
  ...
  {
    table_name: 'returns',
    description: 'Returned orders tracking reverse logistics and refund timestamps',
    columns: [
      { name: 'return_id', type: 'SERIAL', is_pk: true },
      { name: 'order_id', type: 'INTEGER', foreign_table: 'orders', foreign_column: 'order_id' },
  ...
  },
  {
    ...
    description: 'Dense vector catalog used for live Schema RAG semantic similarity embeddings',
    columns: [
      ...
      { name: 'is_foreign_key', type: 'BOOLEAN' },
      { name: 'is_pii', type: 'BOOLEAN' },
